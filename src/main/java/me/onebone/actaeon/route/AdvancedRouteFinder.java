                ...
                if (endNode.equals(node)) {
                    ...
                    while ((node = node.getParent()) != null) {
                        ...
                        if (lastNode.getY() == node.getY() && direction.lengthSquared() > 0) {  //Y不改变
                            WalkableIterator iterator = new WalkableIterator(
                                    this,
                                    entity,
                                    entity.getLevel(),
                                    lastNode.getVector3(),
                                    direction,
                                    entity.getWidth(),
                                    (int) lastNode.getVector3().distance(node.getVector3()) + 1
                            );
                    ...
                    if (highestUnder != null) {
                        Node realDestinationNode = new Node(destination.getX(), highestUnder.getY() + 1.0, destination.getZ());
                        realDestinationNode.setParent(null);
